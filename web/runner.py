...
def _cleanup(db) -> None:
    ...
    cutoff = _dt.now(_tz.utc).replace(tzinfo=None)
    ...
    deleted_links = (
        db.query(MagicLink)
        .filter(or_(MagicLink.used_at.isnot(None), MagicLink.expires_at < cutoff))
        .delete(synchronize_session=False)
    )
    db.commit()
    logger.info(
        "cleanup_done deleted_sessions=%d deleted_magic_links=%d",
        deleted_sessions,
        deleted_links,
    )
...
def _run(db):
    ...
    # Only run queries for users who have credits remaining and whose check interval is due.
    # next_check_at=NULL means the query has never been checked — always due.
    ...
    queries = (
        ...
        .filter(
            ...
            or_(Query.next_check_at == None, Query.next_check_at <= now),
        )
        .all()
    )
...
def _handle_result(db, q, user, result_dict, from_email):
    ...
    query_executions_total.labels(trigger="scheduled", answer=answer).inc()
    ...
    if answer == "YES":
        ...
        try:
            ...
            email_sent = True
            email_sends_total.labels(trigger="scheduled", outcome="success").inc()
            ...
            email_sends_total.labels(trigger="scheduled", outcome="error").inc()
    else:
...
def _run_sequential(client, db, queries, today, from_email):
    ...
    for q in queries:
        ...
        try:
            ...
            claude_api_calls_total.labels(trigger="scheduled", outcome="api_error").inc()
            query_executions_total.labels(trigger="scheduled", answer="error").inc()
            continue
        ...
        if failure:
            ...
            claude_api_calls_total.labels(trigger="scheduled", outcome="search_failure").inc()
            query_executions_total.labels(trigger="scheduled", answer="error").inc()
            continue
        ...
        try:
            ...
            claude_api_calls_total.labels(trigger="scheduled", outcome="parse_error").inc()
            query_executions_total.labels(trigger="scheduled", answer="error").inc()
            continue

        claude_api_calls_total.labels(trigger="scheduled", outcome="success").inc()
        query_execution_duration_seconds.labels(trigger="scheduled").observe(time.monotonic() - t0)
...
def _run_batch(client, db, queries, today, from_email):
    ...
    for result in client.messages.batches.results(batch.id):
        ...
        claude_api_calls_total.labels(trigger="scheduled", outcome="success").inc()
        # Note: per-query duration not observable in batch mode (results arrive together).
