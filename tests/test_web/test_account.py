...
def test_patch_account_empty_body_does_not_clear_notify_email(auth_client):
    """Sending {} should be a no-op, not silently clear notify_email."""
    auth_client.patch("/auth/account", json={"notify_email": "alerts@example.com"})
    r = auth_client.patch("/auth/account", json={})
    assert r.status_code == 200
    assert r.json()["notify_email"] == "alerts@example.com"
